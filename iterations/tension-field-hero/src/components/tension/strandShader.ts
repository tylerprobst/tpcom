export const strandVertexShader = /* glsl */ `
uniform float uPinch;
uniform float uThick;

varying vec3 vNormal;
varying vec3 vViewPosition;
varying float vAlong;
varying float vFresnel;

void main() {
  vAlong = position.y + 0.5;

  float base = 1.0 - uPinch * pow(1.0 - vAlong, 2.0);
  float tip = 0.5 + vAlong * 0.5;
  float radius = uThick * mix(base, tip, vAlong);

  vec3 p = vec3(position.x * radius, position.y, position.z * radius);

  vec4 mvPosition = modelViewMatrix * vec4(p, 1.0);
  vNormal = normalize(normalMatrix * normal);
  vViewPosition = -mvPosition.xyz;
  vFresnel = pow(1.0 - max(dot(vNormal, normalize(vViewPosition)), 0.0), 2.0);

  gl_Position = projectionMatrix * mvPosition;
}
`;

export const strandFragmentShader = /* glsl */ `
uniform float uPull;

varying vec3 vNormal;
varying vec3 vViewPosition;
varying float vAlong;
varying float vFresnel;

void main() {
  vec3 coreColor = vec3(0.96, 0.92, 0.86);
  vec3 midColor = vec3(0.78, 0.84, 0.92);
  vec3 edgeColor = vec3(0.38, 0.48, 0.56);

  float facing = 1.0 - vFresnel;
  vec3 color = mix(edgeColor, midColor, smoothstep(0.0, 0.65, facing));
  color = mix(color, coreColor, smoothstep(0.3, 1.0, facing) * (0.7 + vAlong * 0.3));
  color += vec3(1.0, 0.9, 0.75) * uPull * 0.25 * (1.0 - vAlong * 0.4);

  gl_FragColor = vec4(color, 1.0);
}
`;